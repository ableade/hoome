...
public class MVNServer {
	...
	public static Set<InetSocketAddress> mavenAddresses;
	...
	public static void init () throws SocketException {
		...
		MVNServer.mavenAddresses =  new HashSet <InetSocketAddress>();
	 ...
	 */
	public static void addAddress (InetSocketAddress anAddress, int type) {
		if(type==Consts.NODE_ADDITION_TYPE) {
			if(MVNServer.addresses.size() < Consts.MAX_BYTE_SIZE) {
				MVNServer.addresses.add(anAddress);
			} } else { 
				if(MVNServer.mavenAddresses.size() < Consts.MAX_BYTE_SIZE) {
					MVNServer.mavenAddresses.add(anAddress);
				}
			}
	}
	 ...
	 */
	public static void populateAddresses (MavenCommon aMaven, int type) {
		if(type==Consts.REQUEST_NODE_TYPE) {
			for(InetSocketAddress address : MVNServer.addresses) {
				aMaven.addAddress(address);
			} }else {
				for(InetSocketAddress address : MVNServer.mavenAddresses) {
					aMaven.addAddress(address);
				}
			}
	}
		...
		if(response == null)  { // if there was no problem receiving proceed
			...
			case Consts.REQUEST_NODE_TYPE:  //if type RN
				if(aRequest.getError() ==0) {
					...
					MVNServer.populateAddresses(response,aRequest.getType());
			...
			case Consts.NODE_ADDITION_TYPE:
				for(InetSocketAddress addr: aRequest.getAddresses()) {
					MVNServer.addAddress(addr, aRequest.getType());
				}
				MVNServer.aLogger.log(Level.INFO, "Received:" + aRequest.toString());
			...
			case Consts.MAVEN_DELETION_TYPE:
				MVNServer.mavenAddresses.removeAll(aRequest.getAddresses());
				MVNServer.aLogger.log(Level.INFO, "Received:" + aRequest.toString());
				break;	
